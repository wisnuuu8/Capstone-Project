...
export async function insertDeteksiMata(detectionData) {
    ...
    const { error } = await supabase
        ...
        .insert([
            {
                user_id: user.id,
                ear_score_avg: detectionData.ear_score_avg, // Ganti blink_rate
                cnn_confidence: detectionData.cnn_confidence, // Tambahkan ini
                head_tilt: detectionData.head_tilt,
                status_mata: detectionData.status_mata,
                inference_time_ms: detectionData.inference_time_ms || 0 // Ganti durasi_sesi
            }
        ]);
